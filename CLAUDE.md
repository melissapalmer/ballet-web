...
- **Volunteer roster is live, multi-tab capable.** The studio's "Backstage Mums" sheet has one tab per shift; each shift's `<li class="shift-item">` declares its tab via `<div class="dynamic-roster" data-gid="…">`. `renderVolunteers()` finds every `.dynamic-roster[data-gid]` and fetches `VOLUNTEER_CSV_BASE&gid={gid}&single=true`. Tabs without data show "Roster being finalised". When the studio shares a new tab's gid, swap the `<!-- TODO -->` placeholder in that shift for a `data-gid` div.
- **Emojis are character-driven.** `CHARACTER_EMOJI_RULES` in `app.js` is an ordered regex list — first match wins. Add new rules at the most-specific position when the studio names new characters in the sheet (e.g. "Yellow Brick Road", "Wicked Witch"). `VOLUNTEER_GROUP_EMOJI` is the fallback when the Character cell is empty / unmatched. The CSS body font-stack lists the emoji fonts (`Apple Color Emoji`, `Segoe UI Emoji`, `Noto Color Emoji`) so each OS picks its native set; there's no legal way to ship Apple's emoji font to non-Apple devices.
