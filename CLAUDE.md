...
- **Volunteer roster is live**. The 1 May entry on Volunteers fetches the studio's published sheet (`VOLUNTEER_CSV_URL` in `app.js` — columns: `Group, Character, Mum, Daughter(s)`) and renders it grouped by `Group`, with class rows under "Class roster" and any group starting with "Special -" under "General". Emojis come from `VOLUNTEER_GROUP_EMOJI` keyed by exact group name (unknown groups fall back to 🎭). When the studio adds a new group, add an emoji entry to keep the visual consistency. 9 May / 15 May / 16 May still show "Roster being finalised" until separate sheets / data exist.
