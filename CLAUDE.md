...
- **Google Form URLs**: shirt order form is wired (real URL); volunteer form is still `#TODO-volunteer-form`. Name-spelling form is closed and the studio handles corrections internally — there is **no** Name spelling card on the Orders tab. Don't re-add one. `grep -n '#TODO-' index.html` lists outstanding placeholders.
- **Shirt orders list is live**. The Orders → Show shirt card pulls from a published Google Sheet CSV (`SHIRT_CSV_URL` in `app.js`) and renders dancer name, grade, size, paid status. Source of truth is the studio's Google Sheet — when she ticks "Paid" or new responses arrive, the site updates within minutes (no commit needed). Don't replace the live list with a static table.
