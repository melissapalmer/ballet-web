...
- **Layout is tab-based** — no scroll-to-anchor sections. Tabs are Home, Schedule, Important Info, Volunteers, Orders, Contact, Studio. URL slug for the Important Info tab stays `#info` (don't rename the slug — only the display label changed).
...
- **Show date is the source of truth.** Saturday 16 May 2026, two performances 12:00 and 15:00 at the Playhouse Drama Theatre, Durban. The countdown in `app.js` aims at the 12:00 show (`SHOW_DATETIME`).
- **Schedule is live too.** The Schedule tab fetches `SCHEDULE_CSV_URL` (a published Google Sheet, no header row) via `parseRawCSV` and `renderSchedule()`. Layout: col A = date heading, col B = time/summary, col C = bullet detail. Rows with empty col A continue the previous shift; bullets render in `<ul class="info-list">`. The studio's sheet is the source of truth — don't reintroduce static shift entries.
- **Backstage rule** (show day): only moms on the volunteer roster are allowed backstage. This warning is appended automatically to any shift whose date matches `/\b16\s*May\b/i` in `renderSchedule()` (since the studio's sheet doesn't include it), and also lives on the Volunteers Sat 16 May sub-tab as a `.backstage-note` block. If you reword it, update both spots.
- **All forms are closed.** Volunteer and name-spelling forms are closed; shirts have been delivered/paid and the Show shirt sub-tab + live shirt-orders list have been removed. Don't re-add a Show shirt card or the `SHIRT_CSV_URL` / `renderShirtOrders` logic.
- **Show info is live too.** The Info tab fetches `INFO_CSV_URL` (a published Google Sheet, no header row) via `parseRawCSV` and `renderInfo()`. Column A is a label/heading; column B is content. **Each top-level heading becomes its own `.info-panel` card**: emoji-prefixed col A (e.g. `🎒 What to Pack`) → "list" card; plain col A (e.g. `Hair:`) → "kv" card; continuation rows (empty col A) fold into the current card; empty rows close a section; non-emoji labels inside a list card render as inline `<strong>` sub-labels (e.g. `For dress rehearsals + show day:`). Continuation lines without any prior section render as an orphan intro paragraph above the cards. Newlines inside cells become `<br>`. The studio edits the sheet, site updates within Google's ~5 min publish-cache window.
- **Volunteer roster is live, multi-tab capable.** The studio's "Backstage Mums" sheet has one tab per shift; the Volunteers tab uses **sub-tabs** (same pattern as Orders) — one sub-tab per shift, each containing a `<div class="dynamic-roster" data-gid="…">`. `renderVolunteers()` finds every `.dynamic-roster[data-gid]` (regardless of which sub-tab is active) and fetches `VOLUNTEER_CSV_BASE&gid={gid}&single=true` on page load. Sub-tabs without data show "Roster being finalised". When the studio shares a new tab's gid, swap the `<!-- TODO -->` placeholder in that shift's `<article class="subtab-content shift-panel">` for a `data-gid` div. **Roster columns**: rehearsal sheets have `Group, Character, Mum, Daughter(s)`; show-day sheets add a `Phone` column between Mum and Daughter. The renderer prefers `Phone` over `Daughter(s)` for the chip's secondary line — automatic, based on what's in each sheet.
...
- **Orders tab uses sub-tabs** (`.subtabs` + `.subtab-content`) — Show photos / Show video. Default is Photos. The pattern is generic in `wireSubTabs()` in `app.js` and is also used by Volunteers.
